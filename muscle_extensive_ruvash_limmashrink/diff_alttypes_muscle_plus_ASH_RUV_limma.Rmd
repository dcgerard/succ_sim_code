...
# Abstract
I compare SUCCOTASH to various competitors under the same alternative scenarios in @stephens2016false. SUCCOTASH generally has superior performance in terms of (1) estimating $\pi_0$, (2) having higher AUC, and (3) having lower MSE. LEAPP does better under some scenarios in terms of estimating $\pi_0$ but has far worse AUC and MSE. SUCCOTASH accurately estimates the average sign error rate, though not in as conservative manner as one might hope in some scenarios. I also look at limma-shrinking the variances in RUVASH. It doesn't change the results too much. It seems to just make the estimates of $\pi_0$ slightly more conservative.
...
method_vec <- c(method_vec, "RUVASHOLS", "RUVASHGLS", "SRUVASHGLS", "LIMRUVASH")
...
for (index in 1:length(ash_lfdr)) {
    ...
    lfdr[[index]] <- lfdr[[index]][, c(1:(ncol(lfdr[[index]]) - 5),
                                       (ncol(lfdr[[index]]) - 3):ncol(lfdr[[index]]),
                                       ncol(lfdr[[index]]) - 4)]
}
...
for (index in 1:length(ash_betahat)) {
    ...
    betahat[[index]] <- betahat[[index]][, c(1:(ncol(betahat[[index]]) - 5),
                               (ncol(betahat[[index]]) - 3):ncol(betahat[[index]]),
                               ncol(betahat[[index]]) - 4)]
}
