...
* RUVgls + limma shrink variances + estimate variance inflation using conrols + MLE to UMVUE motivated scaling + ASH
* RUVASH with t-liklihood.
...
* Limma-shrinking the variances results in slightly more conservative estimates of $\pi_0$.
* Using the t-liklihood had very little effect on the results.
...
## pi0hat plots ------------------------------------------------------------------------
load("../muscle_extensive/pi0hat_muscle.Rd")
load("../muscle_extensive/method_vec.Rd")
load("../muscle_extensive/pi0hat_muscle_ash.Rd")
load("../muscle_extensive/pi0hat_muscle_ruvash.Rd")
load("../muscle_extensive/pi0hat_muscle_sruvash.Rd")
load("../muscle_extensive_ruvash_limmashrink_measure_scales/pi0hat_muscle_ruvash.Rd")
load("pi0hat_muscle_ruvash.Rd")
method_vec <- c(method_vec, "RUVASHOLS", "RUVASHGLS", "SRUVASHGLS", "LIMRUVASH", "RUVASHT")

par_vals <- read.csv("../muscle_extensive/sim_settings.csv")
par_vals2 <- read.csv("par_vals.csv")
ruvash_lim_pi0hat <- ruvash_lim_pi0hat[par_vals2$Nsamp < 40]
ruvash_t_pi0hat <- ruvash_t_pi0hat[par_vals2$Nsamp < 40]

for (index in 1:length(ash_pi0hat)) {
    pi0hat[[index]] <- c(pi0hat[[index]], ash_pi0hat[[index]], ruvash_pi0hat[[index]],
                         sruvash_pi0hat[[index]], ruvash_lim_pi0hat[[index]],
                         ruvash_t_pi0hat[[index]])
}
...
## AUC Boxplots ---------------------------------------------------------------------
rm(list = ls())
load("../muscle_extensive/lfdr_muscle.Rd")
load("../muscle_extensive/lfdr_muscle_ash.Rd")
load("../muscle_extensive/lfdr_muscle_ruvash.Rd")
load("../muscle_extensive/lfdr_muscle_sruvash.Rd")
load("../muscle_extensive_ruvash_limmashrink_measure_scales/lfdr_muscle_ruvash.Rd")
load("lfdr_muscle_ruvash.Rd")

par_vals <- read.csv("../muscle_extensive/sim_settings.csv")
par_vals2 <- read.csv("par_vals.csv")
ruvash_lim_lfdr <- ruvash_lim_lfdr[par_vals2$Nsamp < 40]
ruvash_t_lfdr <- ruvash_t_lfdr[par_vals2$Nsamp < 40]
...
for (index in 1:length(ash_lfdr)) {
    ...
    lfdr[[index]]$LIMRUVASH  <- ruvash_lim_lfdr[[index]][, 1]
    lfdr[[index]]$RUVASHT    <- ruvash_t_lfdr[[index]][, 1]
    lfdr[[index]] <- lfdr[[index]][, c(1:(ncol(lfdr[[index]]) - 6),
                                       (ncol(lfdr[[index]]) - 4):ncol(lfdr[[index]]),
                                       ncol(lfdr[[index]]) - 5)]
}
...
auc_mat <- read.csv("../muscle_extensive/auc_mat.csv")
...
rm(list = ls())
load("../muscle_extensive/betahat_muscle.Rd")
load("../muscle_extensive/betahat_muscle_ash.Rd")
load("../muscle_extensive/betahat_muscle_ruvash.Rd")
load("../muscle_extensive/betahat_muscle_sruvash.Rd")
load("../muscle_extensive_ruvash_limmashrink_measure_scales/betahat_muscle_ruvash.Rd")
load("betahat_muscle_ruvash.Rd")

par_vals <- read.csv("../muscle_extensive/sim_settings.csv")
par_vals2 <- read.csv("par_vals.csv")
ruvash_lim_betahat <- ruvash_lim_betahat[par_vals2$Nsamp < 40]
ruvash_t_betahat <- ruvash_t_betahat[par_vals2$Nsamp < 40]
...
for (index in 1:length(ash_betahat)) {
    ...
    betahat[[index]]$LIMRUVASH <- ruvash_lim_betahat[[index]][, 1]
    betahat[[index]]$RUVASHT <- ruvash_t_betahat[[index]][, 1]
    betahat[[index]] <- betahat[[index]][, c(1:(ncol(betahat[[index]]) - 6),
                               (ncol(betahat[[index]]) - 4):ncol(betahat[[index]]),
                               ncol(betahat[[index]]) - 5)]
}
