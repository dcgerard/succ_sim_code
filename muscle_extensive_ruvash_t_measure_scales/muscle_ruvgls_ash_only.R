one_rep <- function(new_params, current_params) {
    ...
    fit_out$multiplier <- ash_ruv_out$ruv$multiplier
    fit_out$num_sv <- ncol(ash_ruv_out$ruv$alpha)
...
ruvash_t_betahat   <- sout[1, ]
ruvash_t_lfdr      <- sout[2, ]
ruvash_t_pi0hat    <- sout[3, ]
ruvash_t_scale     <- sout[4, ]
ruvash_t_numsv     <- sout[5, ]

save(ruvash_t_betahat, file = "betahat_muscle_ruvash.Rd")
save(ruvash_t_lfdr, file = "lfdr_muscle_ruvash.Rd")
save(ruvash_t_pi0hat, file = "pi0hat_muscle_ruvash.Rd")
save(ruvash_t_scale, file = "scale_muscle_ruvash.Rd")
save(ruvash_t_numsv, file = "numsv_muscle_ruvash.Rd")

write.csv(par_vals, file = "par_vals.csv", row.names = FALSE)


plot_df <- cbind(par_vals, sapply(ruvash_t_pi0hat, c))
colnames(plot_df)[ncol(plot_df)] <- "pi0hat"

subdf <- plot_df[plot_df$alt_type == "skew", ]
p <- ggplot(data = subdf, mapping = aes(y = pi0hat, x = rep(1, length = nrow(subdf))))
p <- p + facet_grid(nullpi~Nsamp) + ylab(expression(hat(pi)[0]))
p <- p + geom_boxplot(size = 0.4, outlier.size = 0.4)
p <- p + ggtitle("pi0hat") + geom_hline(yintercept = c(0.5, 0.9), col = "red", lty = 2)
print(p)
