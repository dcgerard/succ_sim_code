...
ols_se_mat <- read.csv("succ_olsse.csv")

pi0_mat$succ_ols_se <- ols_se_mat$pi0hat
mse_mat$succ_ols_se <- ols_se_mat$mse
auc_mat$succ_ols_se <- ols_se_mat$auc

perm_vec <- c(1:6, ncol(pi0_mat), 7:(ncol(pi0_mat) - 1))
pi0_mat <- pi0_mat[, perm_vec]
mse_mat <- mse_mat[, perm_vec]
auc_mat <- auc_mat[, perm_vec]
...
p <- p + geom_boxplot(size = 0.4, outlier.size = 0.4)
p <- p + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.3))
## p <- p + ylim(0, max(mse_mat[, -c(1:4, 10)], na.rm = TRUE))
p <- p + ylim(0, 0.5)
