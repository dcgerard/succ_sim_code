...
one_rep <- function(new_params, current_params) {
    ...
    succ_out <- succotashr::succotash(Y = Y, X = X, k = num_sv, likelihood = "t",
                                      var_scale = FALSE, two_step = FALSE,
                                      mix_type = "uniform")
    tot.time <- proc.time() - start.time



    mse <- mean((succ_out$betahat - beta_true) ^ 2)
    pi0 <- succ_out$pi0
    auc <- NA
    if(args_val$nullpi < 1) {
        auc <- pROC::roc(predictor = succ_out$lfdr, response = which_null)$auc
    }
...
write.csv(sout, file = "succ_mat.csv", row.names = FALSE)


colnames(sout) <- c("pi0hat", "mse", "auc")
succt_df <- as.data.frame(cbind(sout, par_vals))
save(succt_df, file = "succt_df.Rd")
