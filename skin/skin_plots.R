...
p <- p + ggtitle("Estimates of pi0 When Using Skin Tissue")
print(p)
...
p <- p + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.3))
p <- p + ggtitle("AUC When Using Skin Tissue")
print(p)
...
p <- p + ggtitle("MSE When Using Skin Tissue")
print(p)
dev.off()
